...
"${_launch[@]}" -m py123d_garage.training.train \
	training=py123d_garage.config.presets.python.training.latent_transfuser:ltf_kesai_train \
	compile_mode=max-autotune \
	hydra.run.dir='outputs/train_latent_transfuser_kesai/${now:%Y.%m.%d.%H.%M.%S}' \

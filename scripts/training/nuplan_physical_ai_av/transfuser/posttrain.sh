...
"${_launch[@]}" -m py123d_garage.training.train \
	training=py123d_garage.config.presets.python.training.transfuser:tf_mixed_nuplan_physical_ai_av_trainval \
	compile_mode=max-autotune \
	hydra.run.dir='outputs/posttrain_transfuser_nuplan_physical_ai_av/${now:%Y.%m.%d.%H.%M.%S}' \
	policy_config.transfuser_config.planning_config.use_planning_decoder=true \
	initial_weights_file="'outputs/pretrain_transfuser_nuplan_physical_ai_av/<run timestamp>/model_0014.pth'" \

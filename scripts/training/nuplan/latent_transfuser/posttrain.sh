...
"${_launch[@]}" -m py123d_garage.training.train \
	training=py123d_garage.config.presets.python.training.latent_transfuser:ltf_nuplan_trainval \
	compile_mode=max-autotune \
	hydra.run.dir='outputs/posttrain_latent_transfuser_nuplan/${now:%Y.%m.%d.%H.%M.%S}' \
	policy_config.transfuser_config.planning_config.use_planning_decoder=true \
	initial_weights_file="'outputs/pretrain_latent_transfuser_nuplan/<run timestamp>/model_0062.pth'" \

...
"${_launch[@]}" -m py123d_garage.training.train \
	...
	hydra.run.dir='outputs/posttrain_transfuser_physical_ai_av/${now:%Y.%m.%d.%H.%M.%S}' \
	policy_config.transfuser_config.planning_config.use_planning_decoder=true \
	initial_weights_file="'outputs/pretrain_transfuser_physical_ai_av/<run timestamp>/model_0014.pth'" \

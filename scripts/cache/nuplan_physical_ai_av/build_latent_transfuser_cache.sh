...
for _preset in ltf_nuplan_trainval ltf_physical_ai_av_trainval; do
	python -m py123d_garage.cache.build_cache \
		training@plan="py123d_garage.config.presets.python.training.latent_transfuser:${_preset}" \
		hydra.run.dir="outputs/build_cache_latent_transfuser_${_preset}/shard_${SLURM_ARRAY_TASK_ID:-0}" \

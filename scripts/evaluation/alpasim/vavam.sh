...
_checkpoint_file="$(realpath "${CHECKPOINT_FILE:-outputs/checkpoints/vavam/model.pth}")"
_sensor_rig_file="$(realpath "${SENSOR_RIG_FILE:-$(dirname "${_checkpoint_file}")/sensor_rig_0.yaml}")"
_out_dir="$(realpath -m "${OUTPUT_DIR:-outputs/evaluation/alpasim/vavam/$(date +%F_%H-%M-%S)}")"
...
uv run --project lib/alpasim/alpasim alpasim_wizard ${PRESET:-deploy=local topology=1gpu} \
	driver=garage_vavam driver_source=garage_host \

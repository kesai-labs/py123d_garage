#!/usr/bin/env bash
# Builds the submission image, serves it, and drives one scene with the challenge dev preset: the flow up to submit_image.sh.
# Usage: eval_alpasim_submission.sh <checkpoint dir> <scene id> <output dir> <min score> <min meters>
...
_attempts=3
_tag="ci-$(basename "${_checkpoint_dir}")"
...
_container=""
stop_driver() {
	[[ -n "${_container}" ]] && docker rm -f "${_container}" >/dev/null 2>&1 || true
	_container=""
}
trap stop_driver EXIT

CHECKPOINT_FILE="${_checkpoint_dir}/model.pth" TAG="${_tag}" bash scripts/alpasim_submission/build_docker_image.sh

_drive=""
for _attempt in $(seq 1 "${_attempts}"); do
	_attempt_dir="${_out_dir}/attempt_${_attempt}"
	mkdir -p "${_attempt_dir}/driver"
	...
	_baseport=${_base}
	_port=$((_base + 80))
	_container="${_project}-a${_attempt}-driver"
	...
	_sampler_pid=$!
	OUTPUT_DIR="${_attempt_dir}" TAG="${_tag}" ALPASIM_DRIVER_PORT="${_port}" CONTAINER_NAME="${_container}" \
		COMPOSE_PROJECT_NAME="${_project}-a${_attempt}" \
		timeout "${_drive_timeout_s}" bash scripts/alpasim_submission/smoke_test_image.sh \
		scenes.scene_ids="[${_scene_id}]" wizard.baseport="${_baseport}" || echo "Drive exited with code $?"
	...
	# A killed wizard leaves its services running; the driver container goes with them.
...
echo "Submission ${_tag} on ${_scene_id}: score ${_score}, minimum ${_min_score}; drove ${_meters} m, minimum ${_min_meters} m"
if [[ -n ${GITHUB_STEP_SUMMARY:-} ]]; then
	echo "### submission $(basename "${_checkpoint_dir}") on ${_scene_id}: score ${_score}, drove ${_meters} m, ${_peak}" >>"${GITHUB_STEP_SUMMARY}"
fi

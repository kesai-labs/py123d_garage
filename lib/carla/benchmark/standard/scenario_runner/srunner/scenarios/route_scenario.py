...
from srunner.tools.route_parser import DIST_THRESHOLD, RouteParser
...
class RouteScenario(BasicScenario):
    ...
    def _build_scenarios(
        ...
        for scenario_number, scenario_config in enumerate(scenario_definitions):
            ...
            try:
                ...
                if not debug:
                    print(f"Skipping scenario '{scenario_config.type}' due to setup error: {e}")
                else:
                    traceback.print_exc()
                continue
        ...
        for scenario in self.list_scenarios:
            if scenario.behavior_tree is not None:
                ...
                blackboard_list.append(
                    [scenario.config.route_var_name, scenario.config.trigger_points[0].location, scenario.name],
                )
    ...
    def _create_test_criteria(self):
        ...
        criteria.add_child(
            InRouteTest(self.ego_vehicles[0], route=self.route, offroad_max=30, terminate_on_failure=True),
        )
        criteria.add_child(
            ActorBlockedTest(
        ...
        for scenario in self.list_scenarios:
            ...
            criteria.add_child(self._create_criterion_tree(scenario, scenario_criteria))

...
class OtherLeadingVehicle(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        first_vehicle_transform = carla.Transform(
            first_vehicle_waypoint.transform.location, first_vehicle_waypoint.transform.rotation,
        )
        second_vehicle_transform = carla.Transform(
            second_vehicle_waypoint.transform.location, second_vehicle_waypoint.transform.rotation,
        )
    ...
    def _create_behavior(self):
        ...
        driving_in_same_direction = py_trees.composites.Parallel(
            "All actors driving in same direction", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE,
        )
        ...
        keep_velocity = py_trees.composites.Parallel(
            "Trigger condition for deceleration", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE,
        )

...
class BackgroundBehavior(AtomicBehavior):
    ...
    def _add_junctions_topology(self, route_data):
        ...
        for junction_data in route_data:
            ...
            if self.debug:
                ...
                print(f"> R Entry Lane: {get_lane_key(self._route[junction_data.route_entry_index])}")
                print(f"> R Exit  Lane: {get_lane_key(exit_lane)}")
        ...
        for lane_key in self._road_dict:
            ...
            if source_key in junction.route_entry_keys:
                junction.entry_sources.append(
                    Source(source.wp, source.actors, entry_lane_wp=source.wp, active=source.active),
                )
    ...
    def _initialise_road_behavior(self, road_wps):
        ...
        for wp in road_wps:
            ...
            self._road_dict[get_lane_key(wp)] = Source(prev_wp, actors, active=self._active_road_sources)
            ...
            for source in junction.entry_sources:
                ...
                if distance > self._junction_spawn_dist:
                    ...
                    if (
                        ...
                        and get_lane_key(source.entry_lane_wp) not in junction.route_entry_keys
                    ):
    ...
    def _spawn_actor(self, spawn_wp, ego_dist=0):
        ...
        spawn_transform = carla.Transform(
            spawn_wp.transform.location + carla.Location(z=self._spawn_vertical_shift), spawn_wp.transform.rotation,
        )
    ...
    def _spawn_actors(self, spawn_wps, ego_dist=0):
        ...
        for wp in spawn_wps:
            ...
            spawn_transforms.append(
                carla.Transform(
                    wp.transform.location + carla.Location(z=self._spawn_vertical_shift), wp.transform.rotation,
                ),
            )
    ...
    def _spawn_source_actor(self, source, ego_dist=20):
        ...
        new_transform = carla.Transform(
            source_transform.location + carla.Location(z=self._spawn_vertical_shift), source_transform.rotation,
        )
        actor = CarlaDataProvider.request_new_actor(
            "vehicle.*",
            new_transform,

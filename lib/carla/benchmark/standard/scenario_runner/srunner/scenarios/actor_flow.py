...
from srunner.tools.scenario_helper import generate_target_waypoint_in_route, get_same_dir_lanes
...
def get_interval_parameter(config, name, p_type, default):
    if name in config.other_parameters:
        return [p_type(config.other_parameters[name]["from"]), p_type(config.other_parameters[name]["to"])]
    return default
...
class EnterActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__("EnterActorFlow", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable)
...
class HighwayExit(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__("HighwayExit", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable)
...
class MergerIntoSlowTraffic(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__(
            "MergerIntoSlowTraffic", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable,
        )
    ...
    def _create_behavior(self):
        ...
        for wp in sink_wps:
            root.add_child(
                InTriggerDistanceToLocation(self.ego_vehicles[0], wp.transform.location, self._sink_distance),
            )
        root.add_child(
            ActorFlow(
                source_wp,
...
class InterurbanActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__(
            "InterurbanActorFlow", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable,
        )
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        root.add_child(
            ActorFlow(self._source_wp, self._sink_wp, self._source_dist_interval, self._sink_distance, self._flow_speed),
        )
        root.add_child(ScenarioTimeout(self._scenario_timeout, self.config.name))
...
class InterurbanAdvancedActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__(
            "InterurbanAdvancedActorFlow", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable,
        )

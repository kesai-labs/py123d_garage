...
class ParkingExit(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, debug_mode=False, criteria_enable=True, timeout=180):
        ...
        if self._parking_waypoint is None:
            raise Exception(f"Couldn't find parking point on the {self._direction} side")

        super().__init__("ParkingExit", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable)
    ...
    def _initialize_actors(self, config):
        ...
        front_points = self._parking_waypoint.next(self._front_vehicle_distance)
        if not front_points:
        ...
        behind_points = self._parking_waypoint.previous(self._behind_vehicle_distance)

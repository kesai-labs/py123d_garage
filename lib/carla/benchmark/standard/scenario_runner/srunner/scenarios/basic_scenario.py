...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import UpdateAllActorControls, WaitForever
...
class BasicScenario:
        ...
        if self.criteria_enable:
            ...
            # Lazy mode, but its okay, we'll create the parallel behavior tree for you.
                ...
                self.criteria_tree = py_trees.composites.Parallel(
                    name="Test Criteria", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL,
                )
                ...
                raise ValueError(
                    f"WARNING: Scenario {self.name} couldn't be setup, make sure the criteria is either "
                    "a list or a py_trees.composites.Composite",
                )
    ...
    def _setup_scenario_trigger(self, config):
        ...
        check_name = f"WaitForBlackboardVariable: {config.route_var_name}"
    ...
    def _setup_scenario_end(self, config):
        ...
        name = f"Reset Blackboard Variable: {config.route_var_name} "

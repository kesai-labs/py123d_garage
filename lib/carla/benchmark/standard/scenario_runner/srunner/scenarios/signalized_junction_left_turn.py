...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import ActorFlow, ScenarioTimeout, TrafficLightFreezer
...
class JunctionLeftTurn(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__("JunctionLeftTurn", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable)
    ...
    def _initialize_actors(self, config):
        ...
        if not source_entry_wps:
            raise ValueError(f"Trying to find a lane in the {self._direction} direction but none was found")
...
class SignalizedJunctionLeftTurn(JunctionLeftTurn):
    ...
    def _create_behavior(self):
        ...
        root.add_child(ActorFlow(self._source_wp, self._sink_wp, self._source_dist_interval, 2, self._flow_speed))

...
class OppositeVehicleJunction(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__(
            "OppositeVehicleJunction", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable,
        )
    ...
    def _initialize_actors(self, config):
        ...
        self._spawn_location = carla.Transform(
            source_transform.location + carla.Location(z=0.1), source_transform.rotation,
        )
        ...
        opposite_transform = carla.Transform(
            source_transform.location - carla.Location(z=500), source_transform.rotation,
        )
...
class OppositeVehicleRunningRedLight(OppositeVehicleJunction):
    ...
    def _create_behavior(self):
        ...
        trigger_adversary.add_child(
            InTimeToArrivalToLocation(self.ego_vehicles[0], self._sync_time, self._collision_location),
        )
        trigger_adversary.add_child(
            InTriggerDistanceToLocation(self.ego_vehicles[0], self._collision_location, self._min_trigger_dist),
        )

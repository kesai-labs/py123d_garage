...
from srunner.tools.background_manager import ChangeOppositeBehavior, ChangeRoadBehavior, LeaveSpaceInFront, SetMaxSpeed
...
class Accident(BasicScenario):
    ...
    def _spawn_obstacle(self, wp, blueprint, accident_actor=False):
        ...
        if accident_actor:
            ...
            actor = CarlaDataProvider.request_new_actor(blueprint, spawn_transform, rolename="scenario")
        if not actor:
...
class HazardAtSideLane(BasicScenario):
    ...
    def __init__(
        ...
        super().__init__(
            "HazardAtSideLane", ego_vehicles, config, world, randomize, debug_mode, criteria_enable=criteria_enable,
        )
    ...
    def _create_behavior(self):
        ...
        behavior.add_child(
            InTriggerDistanceToVehicle(self.ego_vehicles[0], self.other_actors[0], self._trigger_distance),
        )
        behavior.add_child(Idle(self._wait_duration))
        if self.route_mode:
            behavior.add_child(SetMaxSpeed(self._max_speed))

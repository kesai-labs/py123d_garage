...
class ChangeLane(BasicScenario):
    ...
    def __init__(
        self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=600,
    ):
        """
    ...
    def _create_behavior(self):
        ...
        endcondition = py_trees.composites.Parallel(
            "Waiting for end position of ego vehicle", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL,
        )
        endcondition_part1 = InTriggerDistanceToVehicle(
            self.other_actors[1], self.ego_vehicles[0], distance=20, name="FinalDistance",
        )

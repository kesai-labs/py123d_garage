...
def get_crossing_point(actor):
    ...
    crossing = carla.Location(
        x=wp_cross.transform.location.x, y=wp_cross.transform.location.y, z=wp_cross.transform.location.z,
    )
...
def get_troad_from_transform(actor_transform):
    ...
    lm_tr = get_offset_transform(
        carla.Transform(lm_lane.transform.location, lm_lane.transform.rotation), lm_lane_offset,
    )
    ...
    distance_from_rm_lane_edge = get_offset_transform(
        carla.Transform(rm_lane.transform.location, rm_lane.transform.rotation), rm_lane_offset,
...
def get_distance_between_actors(
    current, target, distance_type="euclidianDistance", freespace=False, global_planner=None,
):
    """

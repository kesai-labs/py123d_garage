...
def convert_elem_to_transform(elem):
    ...
    return carla.Transform(
        carla.Location(float(elem.attrib.get("x")), float(elem.attrib.get("y")), float(elem.attrib.get("z"))),
        carla.Rotation(roll=0.0, pitch=0.0, yaw=float(elem.attrib.get("yaw"))),
    )
...
class RouteParser:
    ...
    def is_scenario_at_route(trigger_transform, route):
        ...
        def is_trigger_close(trigger_transform, route_transform):
            ...
            return dist < DIST_THRESHOLD and (angle_dist < ANGLE_THRESHOLD or angle_dist > (360 - ANGLE_THRESHOLD))

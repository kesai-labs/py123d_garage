...
class SimpleVehicleControl(BasicControl):
    ...
    def run_step(self):
        ...
        if not self._waypoints:
            ...
            while (
                self._generated_waypoint_list
                and self._generated_waypoint_list[0].location.distance(self._actor.get_location()) < 0.5
            ):
    ...
    def _set_new_velocity(self, next_location):
        ...
        if target_speed < current_speed:
            ...
            if self._max_deceleration is not None:
                target_speed = max(
                    target_speed, current_speed - (current_time - self._last_update) * self._max_deceleration,
                )
        else:
            ...
            if self._max_acceleration is not None:
                tmp_speed = min(
                    target_speed, current_speed + (current_time - self._last_update) * self._max_acceleration,
                )

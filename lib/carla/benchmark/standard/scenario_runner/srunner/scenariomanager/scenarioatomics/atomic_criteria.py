...
class Criterion(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name, actor, optional=False, terminate_on_failure=False):
        super().__init__(name)
...
class DrivenDistanceTest(Criterion):
    ...
    def update(self):
        ...
        if self.actual_value > self.success_value:
            self.test_status = "SUCCESS"
        elif self.acceptable_value is not None and self.actual_value > self.acceptable_value:
...
class AverageVelocityTest(Criterion):
    ...
    def __init__(self, actor, velocity, acceptable_velocity=None, optional=False, name="CheckAverageVelocity"):
        """
...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):  # pylint: disable=too-many-return-statements
        ...
        collision_event.set_message(
            f"Agent collided against object with type={event.other_actor.type_id} and id={event.other_actor.id} at (x={round(actor_location.x, 3)}, y={round(actor_location.y, 3)}, z={round(actor_location.z, 3)})",
        )
            ...
            if linear_speed is not None:
                if linear_speed < self._min_speed and self._time_last_valid_state:
                    if (GameTime.get_time() - self._time_last_valid_state) > self._max_time:
                        ...
                        event.set_message(
                            f"Agent got blocked at (x={round(vehicle_location.x, 3)}, y={round(vehicle_location.y, 3)}, z={round(vehicle_location.z, 3)})",
                        )
...
class OnSidewalkTest(Criterion):
    ...
    def _set_event_message(self, event, location, distance):
        ...
        event.set_message(
            f"{message_start} for about {round(distance, 3)} meters, starting at (x={round(location.x, 3)}, y={round(location.y, 3)}, z={round(location.z, 3)})",
        )
...
class OutsideRouteLanesTest(Criterion):
    ...
    def _set_traffic_event(self):
        ...
        self._traffic_event.set_message(
            f"Agent went outside its route lanes for about {round(self._wrong_distance, 3)} meters ({round(percentage, 2)}% of the completed route)",
        )
...
class WrongLaneTest(Criterion):
    ...
    def _set_event_message(self, event, location, distance, road_id, lane_id):
        ...
        event.set_message(
            f"Agent invaded a lane in opposite direction for {round(distance, 3)} meters, starting at (x={round(location.x, 3)}, y={round(location.y, 3)}, z={round(location.z, 3)}). "
            f"road_id={road_id}, lane_id={lane_id}",
        )
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if off_route:
                ...
                route_deviation_event.set_message(
                    f"Agent deviated from the route at (x={round(location.x, 3)}, y={round(location.y, 3)}, z={round(location.z, 3)})",
                )
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(self, actor, route, name="RouteCompletionTest", terminate_on_failure=False):
        ...
        self._traffic_event.set_message(f"Agent has completed {self.actual_value} of the route")
        self.events.append(self._traffic_event)
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and ve_dir.dot(wp_dir) > 0:
                    ...
                    if self.is_vehicle_crossing_line((tail_close_pt, tail_far_pt), (lft_lane_wp, rgt_lane_wp)):
                        ...
                        red_light_event.set_message(
                            f"Agent ran a red light {traffic_light.id} at (x={round(location.x, 3)}, y={round(location.y, 3)}, z={round(location.z, 3)})",
                        )
        ...
        if not self.is_actor_affected_by_stop(check_wps, self._target_stop_sign):
            if not self._stop_completed and self._last_failed_stop != self._target_stop_sign.id:
                ...
                running_stop_event.set_message(
                    f"Agent ran a stop with id={self._target_stop_sign.id} at (x={round(stop_location.x, 3)}, y={round(stop_location.y, 3)}, z={round(stop_location.z, 3)})",
                )

    ...
    *All behaviors should use this class as parent*
    ...
    def initialise(self):
        ...
        if self._actor is not None:
            try:
                check_attr = operator.attrgetter(f"running_WF_actor_{self._actor.id}")
                terminate_wf = copy.copy(check_attr(py_trees.blackboard.Blackboard()))
                py_trees.blackboard.Blackboard().set(
                    f"terminate_WF_actor_{self._actor.id}", terminate_wf, overwrite=True,
                )
...
class RunScript(AtomicBehavior):
    ...
    def update(self):
        ...
        if not os.path.isfile(path):
            ...
            print(f"Script file does not exists {path}")
        else:
...
class ChangeActorControl(AtomicBehavior):
    ...
    def __init__(self, actor, control_py_module, args, scenario_file_path=None, name="ChangeActorControl"):
        ...
        self._actor_control = ActorControl(
            actor, control_py_module=control_py_module, args=args, scenario_file_path=scenario_file_path,
        )
        ...
        for i, element in enumerate(carla_route_elements):
            if element[1] == "shortest":
                ...
                try:
                    ...
                    print(f"WARNING: No route from {waypoint} to {waypoint_next} - Using direct path instead")
                    route.append(element[0])
                    continue
...
class ChangeActorLateralMotion(AtomicBehavior):
    ...
    def update(self):
        ...
        if not self._plan:
            print(f"{self._actor} couldn't perform the expected lane change")
            return py_trees.common.Status.FAILURE
        ...
        if self._type == "vehicle":
            if not self._forced_speed:
                ...
                self._actor.set_target_velocity(
                    carla.Vector3D(math.cos(yaw) * self._target_velocity, math.sin(yaw) * self._target_velocity, 0),
                )
...
class SyncArrivalWithAgent(AtomicBehavior):
    ...
    def __init__(self, actor, reference_actor, actor_target, reference_target, end_dist=1, name="SyncArrivalWithAgent"):
        """
        Setup required parameters
        """
        super().__init__(name, actor)
    ...
    def initialise(self):
        ...
        self._agent = ConstantVelocityAgent(
            self._actor, map_inst=CarlaDataProvider.get_map(), grp_inst=CarlaDataProvider.get_global_route_planner(),
        )
...
class ConstantVelocityAgentBehavior(AtomicBehavior):
    ...
    def __init__(self, actor, target_location, target_speed=None, opt_dict=None, name="ConstantVelocityAgentBehavior"):
        """
...
class AdaptiveConstantVelocityAgentBehavior(AtomicBehavior):
    ...
    def initialise(self):
        ...
        self._agent = ConstantVelocityAgent(
            self._actor, target_speed, opt_dict=self._opt_dict, map_inst=self._map, grp_inst=self._grp,
        )
...
class WaypointFollower(AtomicBehavior):
    ...
    def initialise(self):
        ...
        try:
            ...
            py_trees.blackboard.Blackboard().set(f"running_WF_actor_{self._actor.id}", active_wf, overwrite=True)
        except AttributeError:
            # no WF is active for this actor
            py_trees.blackboard.Blackboard().set(f"terminate_WF_actor_{self._actor.id}", [], overwrite=True)
            py_trees.blackboard.Blackboard().set(
                f"running_WF_actor_{self._actor.id}", [self._unique_id], overwrite=True,
            )
    ...
    def update(self):
        ...
        check_term = operator.attrgetter(f"terminate_WF_actor_{self._actor.id}")
        terminate_wf = check_term(py_trees.blackboard.Blackboard())

        check_run = operator.attrgetter(f"running_WF_actor_{self._actor.id}")
        ...
        if self._unique_id in terminate_wf:
            ...
            py_trees.blackboard.Blackboard().set(f"terminate_WF_actor_{self._actor.id}", terminate_wf, overwrite=True)
            py_trees.blackboard.Blackboard().set(f"running_WF_actor_{self._actor.id}", active_wf, overwrite=True)
            new_status = py_trees.common.Status.SUCCESS
            return new_status
...
class TrafficLightControllerSetter(AtomicBehavior):
    ...
    def __init__(
        self, traffic_signal_id, state, duration, delay=None, ref_id=None, name="TrafficLightControllerSetter",
    ):
        """
        Init
        """
...
class ActorFlow(AtomicBehavior):
    ...
    def _spawn_actor(self, transform):
        actor = CarlaDataProvider.request_new_actor(
            "vehicle.*", transform, rolename="scenario", attribute_filter=self._attribute_filter, tick=False,
        )
        if actor is None:
            return py_trees.common.Status.RUNNING
...
class InvadingActorFlow(AtomicBehavior):
    ...
    def __init__(
        self, source_wp, sink_wp, reference_actor, spawn_dist, sink_dist=2, offset=0, name="OppositeActorFlow",
    ):
        """
        Setup class members
        """
...
class BicycleFlow(AtomicBehavior):
    ...
    def __init__(
        self, plan, spawn_dist_interval, sink_dist=2, actor_speed=20 / 3.6, initial_actors=False, name="BicycleFlow",
    ):
        """
        Setup class members
        """
    ...
    def _spawn_actor(self, transform):
        ...
        actor = CarlaDataProvider.request_new_actor(
            "vehicle.*", transform, rolename="scenario no lights", attribute_filter={"base_type": "bicycle"}, tick=False,
        )
        if actor is None:
            return
...
class TrafficLightManipulator(AtomicBehavior):
    ...
    def update(self):
        ...
        # 2) Modify the ego lane to yellow when closeby
            ...
            if self.debug:
                print(f"--- Distance until traffic light changes: {distance}")
    ...
    def passed_enough_time(self, time_limit):
        ...
        if self.debug:
            print(f"--- Waited seconds: {self.seconds_waited}")
            ...
            if condition1 and condition2 and condition3:
                ...
                if self._debug:
                    self._world.debug.draw_point(
                        scen_location + carla.Location(z=4), size=0.5, life_time=0.5, color=carla.Color(255, 255, 0),
                    )

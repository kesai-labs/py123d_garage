...
class InTriggerNearCollision(AtomicCondition):
    def __init__(
        self, reference_actor, actor, distance, comparison_operator=operator.lt, name="InTriggerNearCollision",
    ):
        """
...
class OSCStartEndCondition(AtomicCondition):
    ...
    def update(self):
        ...
        if new_status == py_trees.common.Status.RUNNING:
            blackboard_variable_name = f"({self._element_type}){self._element_name}-{self._rule}"
...
class InTimeToArrivalToVehicleSideLane(InTimeToArrivalToLocation):
    ...
    def __init__(
        ...
        super().__init__(actor, time, other_side_location, comparison_operator, name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
...
class WaitUntilInFrontPosition(AtomicCondition):
    ...
    def update(self):
        ...
        close_by = not self._check_distance or location.distance(self._ref_location) < self._distance
...
class WaitForTrafficLightControllerState(AtomicCondition):
    ...
    def __init__(
        self, traffic_signal_id, state, duration, delay=None, ref_id=None, name="WaitForTrafficLightControllerState",
    ):
        """
        Init
        """
...
class WaitForBlackboardVariable(AtomicCondition):
    ...
    def update(self):
        ...
        if value == self._variable_value:
            if self._debug:
                print(f"Blackboard variable {self._variable_name} set to True")
            new_status = py_trees.common.Status.SUCCESS

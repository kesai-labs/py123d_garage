...
class ResultOutputProvider:
    ...
    def create_output_text(self):
        ...
        """
        output = "\n"
        output += f" ======= Results of Scenario: {self._data.scenario_tree.name} ---- {self._result} =======\n"
        ...
        list_statistics = [["Start Time", f"{self._start_time}"]]
        list_statistics.extend([["End Time", f"{self._end_time}"]])
        list_statistics.extend([["System Time", f"{system_time}s"]])
        list_statistics.extend([["Game Time", f"{game_time}s"]])
        list_statistics.extend([["Ratio (Game / System)", f"{ratio}"]])
    ...
    def _write_to_reportjson(self):
        ...
        for criterion in self._data.scenario.get_criteria():
            json_list.append(
                result_dict(
                    criterion.name,
                    f"{criterion.actor.type_id[8:]}-{criterion.actor.id}",
                    criterion.optional,
        ...
        json_list.append(result_dict("Duration", "all", False, timeout, duration, duration <= timeout))
    ...
    def _write_to_junit(self):
        ...
        with open(self._junit, "w", encoding="utf-8") as junit_file:
            ...
            for criterion in self._data.scenario.get_criteria():
                ...
                result_string = (
                    f'    <testcase name="{testcase_name}" status="run" '
                    f'time="0" classname="Scenarios.{self._data.scenario_tree.name}">\n'
                )
                if criterion.test_status != "SUCCESS":
                    result_string += f'      <failure message="{criterion.name}"  type=""><![CDATA[\n'
                    result_string += f"  Actual:   {criterion.actual_value}\n"
                    result_string += f"  Expected: {criterion.success_value}\n"
                    result_string += "\n"
                    result_string += f"  Exact Value: {criterion.name} = {criterion.actual_value}]]></failure>\n"
                else:
                    result_string += f"  Exact Value: {criterion.name} = {criterion.actual_value}\n"
                result_string += "    </testcase>\n"
                junit_file.write(result_string)

            # Handle timeout separately
            result_string = (
                f'    <testcase name="Duration" status="run" time="{self._data.scenario_duration_system}" '
                f'classname="Scenarios.{self._data.scenario_tree.name}">\n'
            )
            if self._data.scenario_duration_game >= self._data.scenario.timeout:
                result_string += '      <failure message="{}"  type=""><![CDATA[\n'.format("Duration")
                result_string += f"  Actual:   {self._data.scenario_duration_game}\n"
                result_string += f"  Expected: {self._data.scenario.timeout}\n"
                result_string += "\n"

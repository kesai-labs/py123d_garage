...
class CarlaDataProvider:  # pylint: disable=too-many-public-methods
    ...
    def register_actor(actor, transform=None):
        ...
        with CarlaDataProvider._lock:
            if actor in CarlaDataProvider._actor_velocity_map:
                raise KeyError(f"Vehicle '{actor.id}' already registered. Cannot register twice!")
            else:
    ...
    def get_velocity(actor):
        ...
        print(f"{__name__}.get_velocity: {actor} not found!")
        return 0.0
    ...
    def get_location(actor):
        ...
        # We are intentionally not throwing here
        # This may cause exception loops in py_trees
        print(f"{__name__}.get_location: {actor} not found!")
        return None
    ...
    def get_transform(actor):
        ...
        # We are intentionally not throwing here
        # This may cause exception loops in py_trees
        print(f"{__name__}.get_transform: {actor} not found!")
        return None
    ...
    def prepare_map():
        ...
        for traffic_light in CarlaDataProvider._world.get_actors().filter("*traffic_light*"):
            if traffic_light not in list(CarlaDataProvider._traffic_light_map):
                ...
                raise KeyError(f"Traffic light '{traffic_light.id}' already registered. Cannot register twice!")
    ...
    def create_blueprint(model, rolename="scenario", color=None, actor_category="car", attribute_filter=None):
        ...
        def check_attribute_value(blueprint, name, value):
            ...
            if attribute_type == carla.ActorAttributeType.Int:
                return blueprint.get_attribute(name).as_int() == value
            if attribute_type == carla.ActorAttributeType.Float:
                return blueprint.get_attribute(name).as_float() == value
            if attribute_type == carla.ActorAttributeType.String:
        ...
        try:
            ...
            if new_model != "":
                bp_filter = new_model
            print(f"WARNING: Actor model {model} not available. Using instead {new_model}")
        ...
        if color:
            if not blueprint.has_attribute("color"):
                print(
                    f"WARNING: Cannot set Color ({color}) for actor {blueprint.id} due to missing blueprint attribute",
                )
            else:
                default_color_rgba = blueprint.get_attribute("color").as_color()
                default_color = f"({default_color_rgba.r}, {default_color_rgba.g}, {default_color_rgba.b})"
                try:
                    blueprint.set_attribute("color", color)
                except ValueError:
                    # Color can't be set for this vehicle
                    print(
                        f"WARNING: Color ({color}) cannot be set for actor {blueprint.id}. Using instead: ({default_color})",
                    )
    ...
    def request_new_actor(
        ...
        if actor is None:
            print(f"WARNING: Cannot spawn actor {model} at position {spawn_point.location}")
            return None
    ...
    def request_new_batch_actors(
        ...
        for i in range(amount):
            ...
            if random_location:
                if CarlaDataProvider._spawn_index >= len(CarlaDataProvider._spawn_points):
                    print(f"No more spawn points to use. Spawned {i + 1} actors out of {amount}")
                    break
                else:
    ...
    def get_actor_by_id(actor_id):
        ...
        print(f"Non-existing actor id {actor_id}")
        return None
    ...
    def remove_actor_by_id(actor_id):
        ...
        print(f"Trying to remove a non-existing actor id {actor_id}")
        return None

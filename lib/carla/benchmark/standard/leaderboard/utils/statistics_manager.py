...
class GlobalRecord:
    def __init__(self):
        ...
        self.meta = {"total_length": 0, "duration_game": 0, "duration_system": 0, "exceptions": []}
...
class StatisticsManager:
    ...
    def sort_records(self):
        ...
        self._results.checkpoint.records.sort(
            key=lambda x: (int(x.route_id.split("_")[1]), int(x.route_id.split("_rep")[-1])),
        )
    ...
    def compute_global_statistics(self):
        ...
        if self._total_routes == 1:
            ...
            for key in global_record.scores_std_dev:
                value = round(
                    math.sqrt(global_record.scores_std_dev[key] / float(self._total_routes - 1)), ROUND_DIGITS,
                )

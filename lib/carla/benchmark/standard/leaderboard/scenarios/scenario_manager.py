...
from leaderboard.autoagents.agent_wrapper import AgentError, AgentWrapperFactory
...
class ScenarioManager:
    ...
    def signal_handler(self, signum, frame):
        ...
        if self._agent_watchdog and not self._agent_watchdog.get_status():
            raise RuntimeError(f"Agent took longer than {self._timeout}s to send its command")
        elif self._watchdog and not self._watchdog.get_status():
            raise RuntimeError(f"The simulation took longer than {self._timeout}s to update")
        self._running = False
        ...
        if self._timestamp_last_run < timestamp.elapsed_seconds and self._running:
            ...
            if self._debug_mode > 1:
                ...
                self._statistics_manager.compute_route_statistics(
                    self.route_index, self.scenario_duration_system, self.scenario_duration_game, failure_message="",
                )
            ...
            self._spectator.set_transform(
                carla.Transform(ego_trans.location + carla.Location(z=70), carla.Rotation(pitch=-90)),
            )

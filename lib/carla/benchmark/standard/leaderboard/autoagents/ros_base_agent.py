...
class ROSLogger:
    def __init__(self, name):
        ...
        self.handler = logging.handlers.RotatingFileHandler(
            os.path.join(logger_path, self.name + ".log"), maxBytes=25 * 1024 * 1024, backupCount=3,
        )
...
class BridgeHelper:
    ...
    def carla2ros_pose(cls, x, y, z, roll, pitch, yaw, to_quat=False):
        ...
        if to_quat:
            out_orientation = cls.rpy2quat(out_orientation["roll"], out_orientation["pitch"], out_orientation["yaw"])
        return {"position": out_position, "orientation": out_orientation}
...
class ROSBaseAgent(AutonomousAgent):
    ...
    def _vehicle_control_cmd_callback(self, control_msg):
        ...
        if self._last_control_timestamp is not None and abs(self._last_control_timestamp - control_timestamp) < EPSILON:
            print(
                f"\033[93mWARNING: A new vehicle command with a repeated timestamp has been received {control_timestamp} .\033[0m",
        ...
        if abs(control_timestamp - carla_timestamp) > EPSILON:
            print(
                f"\033[93mWARNING: Expecting a vehicle command with timestamp {carla_timestamp} but the timestamp received was {control_timestamp} .\033[0m",
                "\033[93mThis vehicle command will be ignored.\033[0m",
                sep=" ",
            )
            return
    ...
    def run_step(self, _, timestamp):
        ...
        if abs(control_timestamp - carla_timestamp) > EPSILON:
            print(
                f"\x1b[93mWARNING: Expecting a vehicle command with timestamp {carla_timestamp} but the timestamp received was {control_timestamp} .\x1b[0m",
            )

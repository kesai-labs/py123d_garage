...
MIN_CARLA_VERSION = Version("0.9.14")
...
class LeaderboardEvaluator:
    ...
    def __init__(self, args, statistics_manager):
        ...
        if str(carla_version) != "leaderboard":
            if carla_version < MIN_CARLA_VERSION:
                raise ImportError(
                    f"CARLA version {MIN_CARLA_VERSION} or newer required. CARLA version found: {carla_version}",
                )
    ...
    def _signal_handler(self, signum, frame):
        ...
        if self._agent_watchdog and not self._agent_watchdog.get_status():
            raise RuntimeError(f"Timeout: Agent took longer than {self.client_timeout}s to setup")
        elif self.manager:
    ...
    def _load_and_wait_for_world(self, args, town):
        ...
        if map_name != town:
            raise Exception(f"The CARLA server uses the wrong map! This scenario requires the use of map {town}")
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        print(f"\n\033[1m========= Preparing {config.name} (repetition {config.repetition_index}) =========\033[0m")
        ...
        try:
            ...
            agent_class_name = self.module_agent.get_entry_point()
            agent_class_obj = getattr(self.module_agent, agent_class_name)

            # Start the ROS1 bridge server only for ROS1 based agents.
            if agent_class_obj.get_ros_version() == 1 and self._ros1_server is None:
        ...
        try:
            ...
            if args.record:
                self.client.start_recorder(f"{args.record}/{config.name}_rep{config.repetition_index}.log")
...
def main():
    ...
    parser.add_argument(
        "--traffic-manager-port", default=8000, type=int, help="Port to use for the TrafficManager (default: 8000)",
    )
    parser.add_argument(
        "--traffic-manager-seed", default=0, type=int, help="Seed used by the TrafficManager (default: 0)",
    )
    parser.add_argument("--debug", type=int, help="Run with debug output", default=0)
    parser.add_argument(
        "--record", type=str, default="", help="Use CARLA recording feature to create a recording of the scenario",
    )
    ...
    parser.add_argument("--routes-subset", default="", type=str, help="Execute a specific set of routes")
